...
export default function SearchTag() {
    ...
    const [searchInfo,setSearchInfo] = useState<SearchInfo>({searchString:'',sortBy:'question'});
    ...
    useAxiosPost(fetchSetSearchResults,'/tag/search',searchInfo,cookies.token,[searchInfo.searchString,searchInfo.sortBy]);
    ...
    <Tabs value={searchInfo.sortBy} onChange={handleTabValue}>
        <Tab value="question" label="Questions"/>
    ...
    <Box className="search-results" sx={{maxHeight:'700px',overflow:'auto'}}>
        {
            searchInfo.searchResults?.map( (item) => {
                return <ListItem key={item.ID}><TagCompact key={item.ID} id={item.ID}></TagCompact>
            </ListItem>;
            })
        }
    </Box>
